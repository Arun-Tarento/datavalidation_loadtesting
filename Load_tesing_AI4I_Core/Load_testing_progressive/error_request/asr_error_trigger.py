#!/usr/bin/env python3
"""
Comprehensive ASR Error Trigger Script - AI4I Core API
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_tesing_AI4I_Core/Load_testing_progressive/error_request/asr_error_trigger.py
"""
...
base_url = os.getenv("ASR_BASE_URL", "https://core-v1.ai4inclusion.org")
...
samples_file = os.path.join(project_root, "load_testing_test_samples/asr/audio_samples.json.example")
try:
    with open(samples_file, 'r') as f:
        data = json.load(f)
        valid_audio = data.get("audio_samples", [""])[0]
except:
    valid_audio = "dGVzdCBhdWRpbyBjb250ZW50"  # Fallback base64
...
print("="*80)
print("ASR ERROR TRIGGER SCRIPT - AI4I Core API")
print("="*80)
print(f"Target: {base_url}/api/v1/asr/inference")
...
for i in range(10):
    payload = {
        "audio": [
            {
                "audioContent": ""  # Empty audio - causes server error
            }
        ],
        "config": {
            "language": {
...
for i in range(20):
    payload = {
        ...
        "config": {
            ...
            "serviceId": f"invalid-asr-service-{i}",  # Invalid service ID
            "audioFormat": "wav",
            "samplingRate": 16000,
            "encoding": "base64"
        },
...
for i in range(5):
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/api/v1/asr/inference/",  # Trailing slash might redirect
            json=payload,
            headers=headers,
            timeout=30,
...
# ============================================================================
print("="*80)
print("SUMMARY - ASR ERROR TRIGGER RESULTS (AI4I Core API)")
print("="*80)

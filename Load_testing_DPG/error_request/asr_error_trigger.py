#!/usr/bin/env python3
"""
Comprehensive ASR Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/asr_error_trigger.py
"""
...
samples_file = os.path.join(project_root, "load_testing_test_samples/ASR/audio_samples.json")
try:
...
print("="*80)
print("ASR ERROR TRIGGER SCRIPT")
print("="*80)
...
for i in range(20):
    payload = {
        "controlConfig": {"dataTracking": True},
        "config": {
            "serviceId": f"invalid-service-{i}",  # Invalid service ID
            "language": {"sourceLanguage": "hi"}
        },
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/asr",
            params={"serviceId": f"invalid-service-{i}"},
            json=payload,
            headers=headers,
            timeout=30
        )
...
for i in range(5):
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/services/inference/asr/",  # Trailing slash might redirect
            params={"serviceId": service_id},
            json=payload,
            headers=headers,
            timeout=30,
...
# ============================================================================
print("="*80)
print("SUMMARY - ASR ERROR TRIGGER RESULTS")
print("="*80)

#!/usr/bin/env python3
"""
Comprehensive NER Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/ner_error_trigger.py
"""
...
samples_file = os.path.join(project_root, "load_testing_test_samples/ner/ner_samples.json")
try:
    with open(samples_file, 'r') as f:
        data = json.load(f)
        valid_text = data.get("ner_samples", [{"source": "राम दिल्ली में रहते हैं"}])[0]["source"]
except:
    valid_text = "राम दिल्ली में रहते हैं"
...
print("="*80)
print("NER ERROR TRIGGER SCRIPT")
print("="*80)
...
for i in range(20):
    payload = {
        "controlConfig": {"dataTracking": True},
        "config": {
            "serviceId": f"invalid-ner-service-{i}",
            "language": {"sourceLanguage": "hi"}
        },
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/ner",
            params={"serviceId": f"invalid-ner-service-{i}"},
            json=payload,
            headers=headers,
            timeout=30
        )
...
# ============================================================================
print("="*80)
print("SUMMARY - NER ERROR TRIGGER RESULTS")
print("="*80)

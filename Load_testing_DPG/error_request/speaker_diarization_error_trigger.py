#!/usr/bin/env python3
"""
Comprehensive Speaker Diarization Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/speaker_diarization_error_trigger.py
"""
...
service_id = "ai4bharat/speaker-diarization"
...
samples_file = os.path.join(project_root, "load_testing_test_samples/speakerdiarization/speakerdiarization.json.example")
try:
...
print("="*80)
print("SPEAKER DIARIZATION ERROR TRIGGER SCRIPT")
print("="*80)
print(f"Target: {base_url}/services/inference/speaker-diarization")
...
for i in range(10):
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/speaker-diarization",
...
for i in range(20):
    payload = {
        "controlConfig": {"dataTracking": True},
        "config": {
            "serviceId": f"invalid-speaker-diarization-{i}"  # Invalid service ID
        },
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/speaker-diarization",
            params={"serviceId": f"invalid-speaker-diarization-{i}"},
            json=payload,
            headers=headers,
            timeout=30
        )
...
for i in range(5):
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/services/inference/speaker-diarization/",  # Trailing slash might redirect
            params={"serviceId": service_id},
            json=payload,
            headers=headers,
            timeout=30,
...
# ============================================================================
print("="*80)
print("SUMMARY - SPEAKER DIARIZATION ERROR TRIGGER RESULTS")
print("="*80)

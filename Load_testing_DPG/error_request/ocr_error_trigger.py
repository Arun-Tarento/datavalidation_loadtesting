#!/usr/bin/env python3
"""
Comprehensive OCR Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/ocr_error_trigger.py
"""
...
# Load valid image sample
samples_file = os.path.join(project_root, "load_testing_test_samples/ocr/ocr_samples.json")
try:
    with open(samples_file, 'r') as f:
        ...
        if valid_samples:
            valid_image = valid_samples[0].get("imageContent", "")
        else:
            valid_image = "dGVzdGltYWdl"  # Fallback
except:
    valid_image = "dGVzdGltYWdl"  # Fallback base64
...
print("="*80)
print("OCR ERROR TRIGGER SCRIPT")
print("="*80)
print(f"Target: {base_url}/services/inference/pipeline/ocr")
...
# ============================================================================
print("📛 Triggering 10 x 500 Server Errors (empty image/invalid data)...")
print("-" * 80)
...
for i in range(10):
    payload = {
        ...
        "inputData": {
            "image": [
                {"imageContent": ""}  # Empty image - causes server error
            ]
        },
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/pipeline/ocr",
            json=payload,
            headers=headers,
            timeout=30
        )
        ...
        "pipelineTasks": [
            {
                "taskType": "ocr",
                "config": {
                    "serviceId": f"invalid-ocr-service-{i}",
                    "language": {"sourceLanguage": "hi"}
                }
            }
        ],
...
# ============================================================================
print("="*80)
print("SUMMARY - OCR ERROR TRIGGER RESULTS")
print("="*80)

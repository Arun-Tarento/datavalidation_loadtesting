"""
Language Diarization Load Shaping Test with Enhanced Metrics - Find Server Capacity

This script gradually increases load and captures detailed per-stage metrics.

Usage:
    locust -f Load_testing_DPG/load_testing_by_shape/language_diarization_load_shape_test_with_metrics.py --host=http://13.204.164.186:8000
...
from shape_config import LanguageDiarizationUser, LanguageDiarizationConfig
...
class StagesShapeWithMetrics(LoadTestShape):
    """
    Load shape with per-stage metrics tracking for Language Diarization
    """
...
def on_test_stop(environment, **kwargs):
    ...
    print("\n" + "="*70)
    print("📊 LANGUAGE DIARIZATION LOAD SHAPE TEST COMPLETED")
    print("="*70)
...
def analyze_breaking_point(metrics: Dict) -> Dict[str, Any]:
    ...
    for stage_name, stage_data in metrics.items():
        ...
        if "(" in stage_name and "users)" in stage_name:
            ...
            # Define healthy thresholds for language diarization
            # Language diarization is compute-intensive, so allow higher latency
...
def build_enhanced_json_output(environment, stage_metrics: Dict, error_codes: Dict, audio_lengths: List[float]) -> Dict[str, Any]:
    ...
    config = LanguageDiarizationConfig()
    ...
    overall_stats = {
        ...
        "test_config": {
            "service_id": config.service_id,
            "endpoint": "/services/inference/language-diarization"
        },
        "overall_statistics": {
...
def analyze_capacity(stage_metrics: Dict) -> Dict[str, Any]:
    ...
    for stage_name, metrics in stage_metrics.items():
        ...
        # Classify stage health (adjusted thresholds for language diarization)
...
def generate_recommendations(stage_metrics: Dict) -> Dict[str, Any]:
    ...
    # Check latency issues (language diarization can take longer)
    for stage_name, metrics in stage_metrics.items():
...
def save_enhanced_results(output: Dict[str, Any]):
    ...
    filename = os.path.join(results_dir, f"language_diarization_load_shape_results_{timestamp}.json")
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("Language Diarization Load Shape Testing with Enhanced Metrics")
    print("="*70)
    ...
    print("\nTo run this test:")
    print("  locust -f Load_testing_DPG/load_testing_by_shape/language_diarization_load_shape_test_with_metrics.py --host=http://13.204.164.186:8000")
    print("\nResults will be saved to:")
    print("  Load_testing_DPG/load_testing_shape_results/language_diarization_load_shape_results_TIMESTAMP.json")
    print("="*70 + "\n")

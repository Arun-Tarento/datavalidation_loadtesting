"""
Audio Language Detection (ALD) Load Shaping Test with Enhanced Metrics - Find Server Capacity

This script gradually increases load and captures detailed per-stage metrics.

Usage:
    locust -f Load_testing_DPG/load_testing_by_shape/ald_load_shape_test_with_metrics.py --host=http://13.204.164.186:8000
...
from shape_config import AudioLanguageDetectionUser, AudioLanguageDetectionConfig
...
class StagesShapeWithMetrics(LoadTestShape):
    """
    Load shape with per-stage metrics tracking for Audio Language Detection
    """
...
def on_test_stop(environment, **kwargs):
    ...
    print("\n" + "="*70)
    print("📊 AUDIO LANGUAGE DETECTION (ALD) LOAD SHAPE TEST COMPLETED")
    print("="*70)
...
def analyze_breaking_point(metrics: Dict) -> Dict[str, Any]:
    ...
    for stage_name, stage_data in metrics.items():
        ...
        if "(" in stage_name and "users)" in stage_name:
            ...
            # Define healthy thresholds for audio language detection
            # ALD is typically fast, so use stricter latency requirements
            is_healthy = error_rate < 5 and avg_latency < 10000  # <5% errors, <10s latency
...
def build_enhanced_json_output(environment, stage_metrics: Dict, error_codes: Dict, audio_lengths: List[float]) -> Dict[str, Any]:
    ...
    config = AudioLanguageDetectionConfig()
    ...
    overall_stats = {
        "test_info": {
            ...
            "service": "Audio Language Detection (ALD)",
        ...
        "test_config": {
            "service_id": config.service_id,
            "endpoint": "/services/inference/audio-lang-detection"
        },
        "overall_statistics": {
...
def analyze_capacity(stage_metrics: Dict) -> Dict[str, Any]:
    ...
    for stage_name, metrics in stage_metrics.items():
        ...
        # Classify stage health (adjusted thresholds for audio language detection)
        if error_rate < 1 and p95_latency < 10000:  # <1% errors, <10s P95
            ...
            if "(" in stage_name and "users)" in stage_name:
                ...
                try:
                    ...
                    pass
        elif error_rate < 10 and p95_latency < 30000:  # <10% errors, <30s P95
...
def generate_recommendations(stage_metrics: Dict) -> Dict[str, Any]:
    ...
    # Check latency issues (ALD should be fast)
    for stage_name, metrics in stage_metrics.items():
...
def save_enhanced_results(output: Dict[str, Any]):
    ...
    filename = os.path.join(results_dir, f"ald_load_shape_results_{timestamp}.json")
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("Audio Language Detection (ALD) Load Shape Testing with Enhanced Metrics")
    print("="*70)
    ...
    print("\nTo run this test:")
    print("  locust -f Load_testing_DPG/load_testing_by_shape/ald_load_shape_test_with_metrics.py --host=http://13.204.164.186:8000")
    print("\nResults will be saved to:")
    print("  Load_testing_DPG/load_testing_shape_results/ald_load_shape_results_TIMESTAMP.json")
    print("="*70 + "\n")

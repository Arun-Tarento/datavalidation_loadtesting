#!/usr/bin/env python3
"""
Comprehensive TLD (Language Detection) Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/tld_error_trigger.py
"""
...
samples_file = os.path.join(project_root, "load_testing_test_samples/tld/tld_samples.json")
try:
    with open(samples_file, 'r') as f:
        data = json.load(f)
        valid_text = data.get("tld_samples", [{"source": "नमस्ते कैसे हो"}])[0]["source"]
except:
    valid_text = "नमस्ते कैसे हो"
...
for i in range(20):
    payload = {
        "controlConfig": {"dataTracking": True},
        "config": {
            "serviceId": f"invalid-tld-service-{i}"
        },
...
# ============================================================================
print("="*80)
print("SUMMARY - TLD ERROR TRIGGER RESULTS")
print("="*80)

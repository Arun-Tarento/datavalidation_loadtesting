#!/usr/bin/env python3
"""
Comprehensive Transliteration Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/transliteration_error_trigger.py
"""
...
samples_file = os.path.join(project_root, "load_testing_test_samples/transliteration/transliteration_samples.json")
try:
    with open(samples_file, 'r') as f:
        data = json.load(f)
        valid_text = data.get("transliteration_samples", [{"source": "namaste"}])[0]["source"]
except:
    valid_text = "namaste"
...
print("="*80)
print("TRANSLITERATION ERROR TRIGGER SCRIPT")
print("="*80)
...
for i in range(20):
    payload = {
        "controlConfig": {"dataTracking": True},
        "config": {
            "serviceId": f"invalid-translit-{i}",
            "language": {
                "sourceLanguage": "en",
                "sourceScriptCode": "Latn",
                "targetLanguage": "hi",
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/transliteration",
            params={"serviceId": f"invalid-translit-{i}"},
            json=payload,
            headers=headers,
            timeout=30
        )
...
# ============================================================================
print("="*80)
print("SUMMARY - TRANSLITERATION ERROR TRIGGER RESULTS")
print("="*80)

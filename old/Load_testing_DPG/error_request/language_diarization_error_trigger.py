#!/usr/bin/env python3
"""
Comprehensive Language Diarization Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/language_diarization_error_trigger.py
"""
...
service_id = "ai4bharat/language-diarization"
...
# Load valid audio sample
samples_file = os.path.join(project_root, "load_testing_test_samples/languagediarization/languagediarization.json.example")
try:
    with open(samples_file, 'r') as f:
        data = json.load(f)
        valid_audio = data.get("audio_samples", [""])[0]
except:
    valid_audio = "UklGRjhPBABXQVZFZm10IBAAAAABAAEAgD4AAAB9AAACABAATElTVBoAAABJTkZPSVNGVA4AAABMYXZmNTguMjkuMTAwAGRhdGHyTgQA"  # Fallback base64
...
print("="*80)
print("LANGUAGE DIARIZATION ERROR TRIGGER SCRIPT")
print("="*80)
print(f"Target: {base_url}/services/inference/language-diarization")
...
# ============================================================================
print("📛 Triggering 10 x 500 Server Errors (empty/malformed audio)...")
print("-" * 80)
...
for i in range(10):
    payload = {
        ...
        "audio": [{"audioContent": ""}]  # Empty audio - causes server error
    }

    try:
        response = requests.post(
            f"{base_url}/services/inference/language-diarization",
...
for i in range(20):
    payload = {
        "controlConfig": {"dataTracking": True},
        "config": {
            "serviceId": f"invalid-language-diarization-{i}"  # Invalid service ID
        },
        "audio": [{"audioContent": valid_audio}]
    }

    try:
        response = requests.post(
            f"{base_url}/services/inference/language-diarization",
            params={"serviceId": f"invalid-language-diarization-{i}"},
            json=payload,
            headers=headers,
            timeout=30
        )
...
# ============================================================================
# 3. TRIGGER 5 x 300 REDIRECTS (if API supports it)
# ============================================================================
...
for i in range(5):
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/services/inference/language-diarization/",  # Trailing slash might redirect
            params={"serviceId": service_id},
            json=payload,
            headers=headers,
            timeout=30,
...
# ============================================================================
# 4. TRIGGER 5 x 200 SUCCESS (Valid requests for baseline)
# ============================================================================
...
# ============================================================================
print("="*80)
print("SUMMARY - LANGUAGE DIARIZATION ERROR TRIGGER RESULTS")
print("="*80)

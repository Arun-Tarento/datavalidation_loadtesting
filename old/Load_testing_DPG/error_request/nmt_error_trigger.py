#!/usr/bin/env python3
"""
Comprehensive NMT Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/nmt_error_trigger.py
"""
...
service_id = os.getenv("NMT_SERVICE_ID", "ai4bharat/indictrans2")
...
print("="*80)
print("NMT ERROR TRIGGER SCRIPT")
print("="*80)
...
for i in range(10):
    payload = {
        ...
        "input": [{"source": ""}]  # Empty text - may cause server error
    }
...
for i in range(20):
    payload = {
        "controlConfig": {"dataTracking": True},
        "config": {
            "serviceId": f"invalid-nmt-service-{i}",
            "language": {
                "sourceLanguage": "en",
                "targetLanguage": "hi"
            }
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/translation",
            params={"serviceId": f"invalid-nmt-service-{i}"},
            json=payload,
            headers=headers,
            timeout=30
        )
...
# ============================================================================
print("="*80)
print("SUMMARY - NMT ERROR TRIGGER RESULTS")
print("="*80)

#!/usr/bin/env python3
"""
Comprehensive Audio Language Detection (ALD) Error Trigger Script
Triggers: 10x500, 20x400, 5x300, 5x200 responses
Usage: python3 Load_testing_DPG/error_request/ald_error_trigger.py
"""
...
service_id = "ai4bharat/audio-lang-detection"
...
samples_file = os.path.join(project_root, "load_testing_test_samples/ald/ald.json.example")
try:
...
print("="*80)
print("AUDIO LANGUAGE DETECTION (ALD) ERROR TRIGGER SCRIPT")
print("="*80)
print(f"Target: {base_url}/services/inference/audio-lang-detection")
...
for i in range(10):
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/audio-lang-detection",
...
for i in range(20):
    payload = {
        "controlConfig": {"dataTracking": True},
        "config": {
            "serviceId": f"invalid-ald-service-{i}"  # Invalid service ID
        },
    ...
    try:
        response = requests.post(
            f"{base_url}/services/inference/audio-lang-detection",
            params={"serviceId": f"invalid-ald-service-{i}"},
            json=payload,
            headers=headers,
            timeout=30
        )
...
for i in range(5):
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/services/inference/audio-lang-detection/",  # Trailing slash might redirect
            params={"serviceId": service_id},
            json=payload,
            headers=headers,
            timeout=30,
...
# ============================================================================
print("="*80)
print("SUMMARY - AUDIO LANGUAGE DETECTION (ALD) ERROR TRIGGER RESULTS")
print("="*80)
